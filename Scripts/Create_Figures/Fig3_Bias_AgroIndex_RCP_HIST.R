...
folder_data <- "CordexDev_RCP8_BC"
folder_data_ref <- 'CordexDev_CV2_BC'
...
# Generamos (si no existe) la carpeta que contendrá las figuras
path_filename_data <- file.path(path_Datos_save, folder_saved, "Fig3")
if (!file.exists(path_filename_data)) {
  dir.create(path_filename_data)
  message(paste0("==> Se ha creado la carpeta: ", (path_filename_data)))
}
...
# Generamos (si no existe) la carpeta que contendrá las figuras
path_filename_data <- file.path(path_Datos_save, folder_saved, "Fig3", index.code.i)
if (!file.exists(path_filename_data)) {
  dir.create(path_filename_data)
  message(paste0("==> Se ha creado la carpeta: ", (path_filename_data)))
}
...
for (path.model.i in model_folder_data) {
  ...
  # Modelo de referencia asociado
  # ----------------------------------------------------------------------------
  
  if (grepl("historical", cordex.model.i)) {
    cordex.model.ref.i <- gsub("historical", "rcp85", cordex.model.i)
  }
  if (grepl("rcp85", cordex.model.i)) {
    cordex.model.ref.i <- gsub("rcp85", "historical", cordex.model.i)
  }
  
  # Obtenemos parte de los metadatos a partir del nombre completo del modelo
  df.meta.ref.i <- subset(list.model.cordex, name_model == cordex.model.ref.i)
  
  # Comprobamos que existe un modelo de referencia asociado al modelo
  if (nrow(df.meta.ref.i) == 0) {
    # Pasamos al siguiente modelo histórico
    next
  }
  
  # Nº del modelo de referencia
  N_model.ref.i <- df.meta.ref.i$N_model
  
  # Path local del modelo referencia
  path.model.ref.i <- model_folder_ref[which(basename(model_folder_ref) == N_model.ref.i)]
  
  # message("El modelo '", N_model.data.i, "' tiene asociado el modelo '", N_model.ref.i, "' (modelo de referencia)")
  ...
  model_data_list[[N_model.data.i]] <- list()
  model_ref_list[[N_model.ref.i]] <- list()
  
  # Guardamos el orden de los nombres y Nº de modelos
  name_model_ref <- c(name_model_ref, cordex.model.ref.i)
  number_model_ref <- c(number_model_ref, N_model.ref.i)
  name_model_data <- c(name_model_data, cordex.model.i)
  number_model_data <- c(number_model_data, N_model.data.i)
  
  tryCatch(
    {
      message(paste0("\n============================ ", N_model.data.i, " y ", N_model.ref.i, " ============================="))
      message("==================================================================================")
      
      # Lista de archivos .Rdata en el directorio de "agroindex_..." donde se encuentran los archivos de los índices
      filesRdata <- list.files(file.path(path.model.i, paste0("agroindex_", N_model.data.i)), pattern = "\\.Rdata$", full.names = TRUE)
      filesRdata.ref <- list.files(file.path(path.model.ref.i, paste0("agroindex_", N_model.ref.i)), pattern = "\\.Rdata$", full.names = TRUE)
      ...
      for (BC_used in BC_methods) {
        ...
        index.files.ref <- subset(filesRdata.ref, grepl(pattern, basename(filesRdata.ref)) & !grepl("_list",  basename(filesRdata.ref)))
        
        # Le damos otra oportunidad sin especificar la Cross-Validation utilizada
        if (length(index.files) == 0) {
          pattern <- paste0("^", index.code.i, ".*", toupper(BC_used))
          index.files <- subset(filesRdata, grepl(pattern, basename(filesRdata)) & !grepl("_list",  basename(filesRdata)))
          if (length(index.files) == 0) {
            # Pasamos al siguiente modelo histórico
            message(paste0("No hay archivos .Rdata en la carpeta para el método '", basename(path.model.i), "' y ", index.code.i, " + ", toupper(BC_used), " + ", CV_used))
            next
          }
        }
        ...
        model_data_list[[N_model.data.i]][[BC_used]] <- data_all
        model_ref_list[[N_model.ref.i]][[BC_used]] <- data_all.ref
        ...
        if (isTRUE(plot_indexmap)) {
          ...
          # Guardamos la imagen
          base_name <- paste0("Fig3_", basename(path.model.i), "_", index.code.i, paste0(c("", BC_used, CV_used), collapse = "_"), "_Vrcp_hist.Rdata")
          filename_fig1 <- file.path(path_filename_data, gsub(".Rdata", img.ext, base_name))
          fig1 <- plot_custom_map(climatology(model_data_list[[N_model.data.i]][[BC_used]]),
                                  name_title = paste0(basename(path.model.i), " - ", index.code.i, paste0(c("", BC_used, CV_used), collapse = " - ")),
                                  min.value = min.value, max.value = max.value, name.palette = "RdBu"
          )
          if (img.ext == ".png") {
            png(filename_fig1)
          } else {
          ...
          # Modelo de referencia
          # -----------------------------------------------------------------------------------
          message(paste0("Se pinta el mapa de '",index.code.i, "' para ", basename(path.model.ref.i)))
          
          # Guardamos la imagen
          base_name <- paste0("Fig3_", basename(path.model.ref.i), "_", index.code.i, paste0(c("", BC_used, CV_used), collapse = "_"), "_Vrcp_hist.Rdata")
          filename_fig1 <- file.path(path_filename_data, gsub(".Rdata", img.ext, base_name))
          fig1 <- plot_custom_map(climatology(model_ref_list[[N_model.ref.i]][[BC_used]]),
                                  name_title = paste0(basename(path.model.ref.i), " - ", index.code.i, paste0(c("", BC_used, CV_used), collapse = " - ")),
                                  min.value = min.value, max.value = max.value, name.palette = "RdBu"
          )
          if (img.ext == ".png") {
            png(filename_fig1)
...
for (i in names(model_data_list)) {
  ...
  position <- which(number_model_data == i)
  j <- number_model_ref[position]
  
  bias_list[[i]] <- list()
  
  for (BC_used in BC_methods) {
    bias_list[[i]][[BC_used]] <- gridArithmetics(suppressMessages(climatology(model_data_list[[i]][[BC_used]])), 
                                                 suppressMessages(climatology(model_ref_list[[j]][[BC_used]])), 
                                                 suppressMessages(climatology(model_ref_list[[j]][[BC_used]])), C,  
                                                 operator = c("-", "/", "*"), 
                                                 template = NULL)
    ...
    if (isTRUE(plot_indexmap)) {
      ...
      # Guardamos la imagen
      base_name <- paste0("Fig3_Relative-Bias-", i, "_", j, "_", index.code.i, paste0(c("", BC_used, CV_used), collapse = "_"), ".Rdata")
      filename_fig1 <- file.path(path_filename_data, gsub(".Rdata", img.ext, base_name))
...
model_order <- c("model_26", "model_41", "model_25", "model_40") # number_model_data
...
seq_used <- round(seq(clim_min, clim_max, 2*clim/cmap.nbins), 2)

if (index.code.i %in% c("BEDD", "HI")) {
  clim_max <- clim
  clim_min <- 0 # - clim
  seq_used <- round(seq(clim_min, clim_max, abs(clim_min - clim_max)/cmap.nbins), 2)
  cmap.delta = (colorRampPalette(brewer.pal(9, "OrRd"))(cmap.nbins))  # color palette for biases (red-white-blue)
}
...
message(paste0("Fig3_", index.code.i))
