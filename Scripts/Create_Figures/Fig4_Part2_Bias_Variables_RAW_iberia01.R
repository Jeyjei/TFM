...
model_folder_data <- model_folder_data[grepl(paste0("^", "model_", "(3|4|21|20)$"), basename(model_folder_data), ignore.case = TRUE)] # ("^" ,"model_1", "$")
    ...
    tryCatch(
      {
        ...
        for (BC_used in BC_methods) {
          ...
          index.files.ref <- subset(filesRdata.ref, grepl(pattern_iberia, basename(filesRdata.ref)) & !grepl("_list", basename(filesRdata.ref)))
          ...
          if (option_convert) {
            ...
            if (variable.i %in% c("tasmin", "tasmax", "tas")) {
              ...
              filename_fig1 <- file.path(path_filename_data, paste0("00_", variable.i, "_", int_years[1], "-", int_years[length(int_years)], "_", BC_used ,"_02_newUnits", img.ext))
...
for (variable.i in variable_list) {
  ...
  for (model_j in 1:length(model_order)) {
    ...
    for (BC_used in BC_methods) {
      ...
      grid_list_arg[[paste0(letters[j], " ", sort_names[model_j], " (", toupper(BC_name), ")")]] <- var_bias_list[[variable.i]][[model_order[model_j]]][[BC_used]]
      time_plot[[paste0(sort_names[model_j], " (", toupper(BC_name), ")")]] <- var_time_list[[variable.i]][[model_order[model_j]]][[BC_used]]$Data
      
      if (variable.i == "tasmin") {
        tasmin_grid_list[[paste0(letters[j], " ", sort_names[model_j], " (", toupper(BC_used), ")")]] <- tasmin_bias_p5[[model_order[model_j]]][[BC_used]]
      }
      
      subtitle_plot <- c(subtitle_plot, paste0(sort_names[model_j], " (", toupper(BC_name), ")"))
...
save(var_time_list_ord, file = file.path(dirname(path_filename_data), paste0("fig4_evolucion_temporal", "_S", paste0(c(min(season, na.rm = T), max(season, na.rm = T)), collapse = "-"), "_RAW.Rdata")))
save(var_grid_list_arg, file = file.path(dirname(path_filename_data), paste0("fig4_relative_bias", "_S", paste0(c(min(season, na.rm = T), max(season, na.rm = T)), collapse = "-"), "_RAW.Rdata")))
save(tasmin_grid_list, file = file.path(dirname(path_filename_data), paste0("fig4_relative_bias_tasmin-p5", "_S", paste0(c(min(season, na.rm = T), max(season, na.rm = T)), collapse = "-"), "_RAW.Rdata")))
...
# ========== LA SIGUIENTE PARTE DEL CÓDIGO NO ESTÁ 100% ADAPTADA ===============
# ==============================================================================
...
fig1 <- violinPlot(grid_plot,
           ...
           bwplot.custom = list(
             ...
             key = list( # Leyenda
               space = "bottom", cex = 1,
               text = list(toupper(subtitle_plot)),
               columns = 3,
               points = list(col = colours[-5], pch = 19)
             )
           )
)
...
img.ext = ".pdf"
base_name <- paste0("Fig4_Relative-Bias-iberia01_", variable.i, "_S", paste0(c(min(season, na.rm = T), max(season, na.rm = T)), collapse = "-"), "_V1_RAW.Rdata")
