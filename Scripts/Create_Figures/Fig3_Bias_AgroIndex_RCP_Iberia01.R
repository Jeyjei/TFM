  ...
  tryCatch(
    {
      ...
      for (BC_used in BC_methods) {
        ...
        model_data_list[[N_model.i]][[BC_used]] <- (data_all)
        ...
        if (isTRUE(plot_indexmap)) {
          ...
          # Guardamos la imagen
          base_name <- paste0("Fig3_", basename(path.model.i), "_", index.code.i, paste0(c("", BC_used), collapse = "_"), "_Vrcp_iberia.Rdata")
          ...
          fig1 <- plot_custom_map(climatology(model_data_list[[N_model.i]][[BC_used]]),
            name_title = paste0(basename(path.model.i), " - ", index.code.i, paste0(c("", BC_used), collapse = " - ")),
  ...
  tryCatch(
    {
      ...
      if (isTRUE(plot_indexmap)) {
        ...
        # Guardamos la imagen
        base_name <- paste0("Fig3_", basename(path.model.i), "_", index.code.i, "_Vrcp_iberia.Rdata")
        filename_fig1 <- file.path(path_filename_data, gsub(".Rdata", img.ext, base_name))
        fig1 <- plot_custom_map(climatology(model_ref_list[[N_model.ref.i]]),
          name_title = paste0(basename(path.model.i), " - ", index.code.i),
          min.value = min.value, max.value = max.value
        )
        if (img.ext == ".png") {
          png(filename_fig1)
...
for (i in names(model_data_list)) {
  ...
  for (BC_used in BC_methods) {
    ...
    if (isTRUE(plot_indexmap)) {
      ...
      # Guardamos la imagen
      base_name <- paste0("Fig3_Relative-Bias-iberia01_", i, "_", index.code.i, paste0(c("", BC_used, CV_used), collapse = "_"), ".Rdata")
      filename_fig1 <- file.path(path_filename_data, gsub(".Rdata", img.ext, base_name))
