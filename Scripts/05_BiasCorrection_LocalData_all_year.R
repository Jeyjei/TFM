...
list.variables.bias <- list(c("tas", "tasmin", "tasmax", "pr")) # list("tas", "tasmin", "tasmax", "pr")
...
model_folder_hist <- model_folder[grepl(paste0("^", "model_", "(3|4|21|20)$"), basename(model_folder), ignore.case = TRUE)] # (3|4|20|21)
          ...
          tryCatch(
            {
              ...
              for (i.mode in unique(c("same.historical", type_newdata))) {
                ...
                for (variable_i in variables.bias.i) {
                  ...
                  filename_fig1 <- file.path(path_filename_fig1, paste0("05_", variable_i, "_", int_years[1], "-", int_years[length(int_years)], "_01_original", img.ext))
                  ...
                  fig1 <- plot_custom_map(climatology(variable_i.model),
                    name_title = paste0(N_model.hist.i, ": ", variable_i, " (", getGridUnits(variable_i.model), ") de ", int_years[1], " a ", int_years[length(int_years)])
                  )
                  if (img.ext == ".png") {
                    png(filename_fig1)
                  } else {
                  ...
                  if (option_convert) {
                    ...
                    if (variable_i %in% c("tasmin", "tasmax", "tas")) {
                      ...
                      # Ploteamos la imagen de los datos
                      filename_fig1 <- file.path(path_filename_fig1, paste0("05_", variable_i, "_", int_years[1], "-", int_years[length(int_years)], "_02_newUnits", img.ext))
                      # fig1 <- spatialPlot(climatology(variable_i.model),
                      #   backdrop.theme = "coastline", rev.colors = TRUE,
                      #   main = paste0(N_model.hist.i, ": ", variable_i, " (", getGridUnits(variable_i.model), ") de ", int_years[1], " a ", int_years[length(int_years)])
                      # )
              ...
              #' Calculamos el bias entre el modelo sin corregir y las observaciones
              #' ----------------------------------------------------------------------------------
              ...
              if (is.null(season)) {
                for (variable_i in variables.bias.i) {
                  ...
                  bias <- suppressMessages(gridArithmetics(climatology(x_list[[variable_i]]), climatology(y_list[[variable_i]]), operator = "-"))

                  # Guardamos la imagen
                  base_name <- paste0("05_Bias_", variable_i, "_", int_years[1], "-", int_years[length(int_years)], "_sin_corregir")
                  filename_fig1 <- file.path(path_filename_fig1, paste0(base_name, img.ext))
                  fig1 <- plot_custom_map(bias,
                    name_title = paste0(
                      basename(path.model.i), ": Sesgo de ", variable_i, " (", attr(x_list[[variable_i]]$Variable, "units"), ") de ",
                      int_years[1], " a ", int_years[length(int_years)], "\n Modelo sin corregir. ", paste0(c("", season_i), collapse="Season ")
                    )
                  )
                  if (img.ext == ".png") {
                    png(filename_fig1)
                  ...
                  # Guardamos el Bias del modelo antes de corregirlo
                  save(bias, file = file.path(dirname(path_filename_data), base_name))
                  message("\nSave '", file.path(dirname(path_filename_data), base_name), "'\n")
                  rm(bias, base_name)
                  gc()
                }
              }
              ...
              # Los datos del modelo se interpolan (si fuera necesario) con la malla de la observación
